...
kabouter.nvim targets *only* the latest
...
> `https://github.com/<your_github_username>/kabouter.nvim.git`

You likely want to remove `lazy-lock.json` from your fork's `.gitignore` file
too - it's ignored in the kabouter repo to make maintenance easier, but it's
...
```sh
git clone https://github.com/nvim-lua/kabouter.nvim.git "${XDG_CONFIG_HOME:-$HOME/.config}"/nvim
```
...
```
git clone https://github.com/nvim-lua/kabouter.nvim.git "%localappdata%\nvim"
```

If you're using `powershell.exe`

```
git clone https://github.com/nvim-lua/kabouter.nvim.git "${env:LOCALAPPDATA}\nvim"
```
...
* Can I keep my existing configuration in parallel to kabouter?
  * Yes! You can use [NVIM_APPNAME](https://neovim.io/doc/user/starting.html#%24NVIM_APPNAME)`=nvim-NAME`
    to maintain multiple configurations. For example, you can install the kabouter
    configuration in `~/.config/nvim-kabouter` and create an alias:
    ```
    alias nvim-kabouter='NVIM_APPNAME="nvim-kabouter" nvim'
    ```
    When you run Neovim using `nvim-kabouter` alias it will use the alternative
    config directory and the matching local directory
    `~/.local/share/nvim-kabouter`. You can apply this approach to any Neovim
    distribution that you would like to try out.
* What if I want to "uninstall" this configuration:
  * See [lazy.nvim uninstall](https://lazy.folke.io/usage#-uninstalling) information
* Why is the kabouter `init.lua` a single file? Wouldn't it make sense to split it into multiple files?
  * The main purpose of kabouter is to serve as a teaching tool and a reference
    configuration that someone can easily use to `git clone` as a basis for their own.
    As you progress in learning Neovim and Lua, you might consider splitting `init.lua`
    into smaller parts. A fork of kabouter that does this while maintaining the
    same functionality is available here:
    * [kabouter-modular.nvim](https://github.com/dam9000/kabouter-modular.nvim)
  * Discussions on this topic can be found here:
    * [Restructure the configuration](https://github.com/nvim-lua/kabouter.nvim/issues/218)
    * [Reorganize init.lua into a multi-file setup](https://github.com/nvim-lua/kabouter.nvim/pull/473)

### Install Recipes

Below you can find OS specific install instructions for Neovim and dependencies.

After installing all the dependencies continue with the [Install kabouter](#install-kabouter) step.
...
[here](https://github.com/nvim-lua/kabouter.nvim/issues/1583).
