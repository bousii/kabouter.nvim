...
require('lazy').setup({
    ...
    config = function()
      require('mason-tool-installer').setup {
        ensure_installed = {
          ...
          'rust-analyzer',
          'marksman',
          -- Linters / formatters (not LSP servers)
          'stylua',
          'actionlint',
          'mesonlsp',
          'checkmake',
  ...
  { -- Java
    'nvim-java/nvim-java',
    config = function() require('java').setup { spring_boot_tools = { enable = false } } end,
  },
...
-- [[ LSP Configuration ]]
-- Global LSP attach: keymaps and features for every server
vim.api.nvim_create_autocmd('LspAttach', {
...
-- Global capabilities from blink.cmp, applied to all servers
vim.lsp.config('*', {
...
-- gdscript (special: no Mason, uses Godot's built-in server)
...
-- jdtls (java) with capstone project libraries
...
-- [[ Godot project detection & pipe server ]]
